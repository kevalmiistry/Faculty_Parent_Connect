...
$resu = mysqli_query($link, "Select * from faculty where dname='$dept' LIMIT " . $this_page_first_result . ',' .  $results_per_page);
...
if (isset($_POST['filter'])) {
    $post = $_POST['post'];
    if ($post == 'post') {
        $resu = mysqli_query($link, "Select * from faculty where dname='$dept'");
    } else {
        $resu = mysqli_query($link, "Select * from faculty where pname='$post' AND dname='$dept'");
    }
    $num = mysqli_num_rows($resu);
    $n = 0;
}
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr class="tr-shadow" enctype="multipart/form-data">
                                                <td><a href="photoupdate.php?phid=<?php echo $row['fid'] ?>&for=<?php echo "faculty"; ?>"><img class="image" src="../faculty_photos/<?php echo $row['photo']; ?>"></a></td>
                                                ...
                                                <td>
                                                    <div class="table-data-feature">
                                                        <button class="item" data-toggle="tooltip" data-placement="top" title="Edit">
                                                            <a href="faculty_edit.php?fid=<?php echo $row['fid'] ?>"><i class="zmdi zmdi-edit"></i></a>
                                                        </button>
                                                        <button class="item" data-toggle="tooltip" data-placement="top" title="Delete">
                                                            <a href="faculty_delete.php?fid=<?php echo $row['fid'] ?>"><i class="zmdi zmdi-delete"></i></a>
                                                        </button>
                                                        <button class="item" data-toggle="tooltip" data-placement="top" title="View" data-original-title="More" aria-describedby="tooltip181541">
                                                            <a href="faculty_display.php?fid=<?php echo $row['fid'] ?>"><i class="zmdi zmdi-more"></i></a>
                                                        </button>
                                                    </div>
                                                </td>
                                            </tr>
                                                ...
                                                <td>
                                                    <div class="page_numbers" <?php if (isset($_POST['filter'])) {
                                                        ...
                                                        for ($pagex = 1; $pagex <= $number_of_pages; $pagex++) {
                                                            ...
                                                            if ($page == $pagex) {
                                                                echo '<a style="background-color: #4272d7; color:#fff;" href="faculty.php?page=' . $pagex . '">' . '  ' . $pagex . '  ' . '</a> ';
                                                            } else {
