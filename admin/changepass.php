...
if (isset($_POST['Submit'])) {
    ...
    $fetchpass = mysqli_query($link, "select password from admin where username='$user'");
    while ($row = mysqli_fetch_array($fetchpass, MYSQLI_BOTH)) {
        $oldpass = $_POST['oldpass'];
        $newpass = $_POST['newpass'];
        if ($row[0] == md5($oldpass)) {
            $result = mysqli_query($link, "update admin set password=md5('$newpass') where username='$user'");
            $msg = "success";
        } else {
            $msg = "error";
        }
