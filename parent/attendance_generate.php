...
$asem = $_SESSION['sem'];
$ayear = $_SESSION['c_year'];
$erno = $_SESSION['erno'];
?>
<?php
if(isset($_POST['att_gen'])){
                        ...
                        <table class="table table-borderless table-data3" id="tblData">
                            <thead>
                                ...
                                <tr>
                                    ...
                                    $ressub = mysqli_query($link, "select * from subject where sem=$asem AND dname='$dname'");
                                    while ($rowsub = mysqli_fetch_array($ressub, MYSQLI_BOTH)) {
                                        $restotal = mysqli_query($link, "select count(subname) as 'total' from attedence where  sem=$asem AND year=$ayear AND subcode='" . $rowsub['subcode'] . "' AND date between '$start_date' AND '$end_date' AND dname='$dname' AND erno='$erno' group by erno");
                                    ...
                                    $restotal = mysqli_query($link, "select count(subname) as 'total' from attedence where sem=$asem AND year=$ayear AND date between '$start_date' AND '$end_date' AND dname='$dname' AND erno='$erno' group by erno");
                                    $rowtotal = mysqli_fetch_array($restotal, MYSQLI_BOTH);
                            ...
                            <tbody>
                                ...
                                <?php
                                $c = 0;
                                $resstu = mysqli_query($link, "select * from student where sem='" . $asem . "' AND dept='$dname' AND status='ACTIVE' AND erno='$erno' ORDER By erno");
